...
Route::namespace('Api')->name('api.')->group(function(){
	...
	Route::middleware('auth.api:sanctum')->name('user.')->prefix('user')->group(function(){
	    ...
	    Route::middleware(['checkStatusApi'])->group(function(){
            ...
            // Payment
            Route::get('payment/methods', 'PaymentController@depositMethods');
            Route::post('payment/insert', 'PaymentController@depositInsert');
            Route::get('payment/confirm', 'PaymentController@depositConfirm');

            Route::get('payment/manual', 'PaymentController@manualDepositConfirm');
            Route::post('payment/manual', 'PaymentController@manualDepositUpdate');

            Route::get('payment/history', 'UserController@depositHistory');
	    });
	});
});
